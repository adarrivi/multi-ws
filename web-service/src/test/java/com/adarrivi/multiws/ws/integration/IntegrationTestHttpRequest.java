...
public class IntegrationTestHttpRequest {
	...
	public void performJsonGet(String url, JsonDto<?> dto) {
		try {
			resultActions = mockMvc.perform(get(url).contentType(MediaType.APPLICATION_JSON)
	...
	public void performJsonPost(String url, JsonDto<?> dto) {
		try {
			resultActions = mockMvc.perform(post(url).contentType(MediaType.APPLICATION_JSON).content(
					dto.toJsonString()));
		} catch (Exception ex) {
			throw new AssertionError(ex);
		}
	...
	public void assertOkResponse() {
		try {
			...
			Assert.assertFalse(getResponseAsString().isEmpty());
		} catch (Exception ex) {
			throw new AssertionError(ex);
		}
	}
